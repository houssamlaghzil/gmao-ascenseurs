...
- **Vérifications avant toute clôture de tâche :** `npm run lint`, `npx tsc --noEmit`, `npx vitest run` doivent rester verts. **Ne jamais lancer `npm test` seul** : le script `test` de `package.json` est `vitest` sans `run`, qui démarre le mode watch interactif et ne se termine jamais tout seul (constaté en pratique lors de la Tâche 2 des fondations — process resté bloqué ~40 minutes). Un test existant ne se modifie que si son mock doit être étendu (nouvel export), jamais pour masquer une régression.
