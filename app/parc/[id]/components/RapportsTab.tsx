/**
 * Onglet Rapports de la fiche appareil (section 4.2).
 */
...
export default function RapportsTab({ rapports }: { rapports: Rapport[] }) {
    ...
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200 text-sm">
        <thead className="bg-gray-50">
          <tr>
            {['Numéro', 'Type', 'Date', 'Technicien', 'Validation'].map((label) => (
          ...
          {lignes.length === 0 && (
            <tr>
              <td colSpan={5} className="px-4 py-8 text-center text-gray-500">
                Aucun rapport enregistré pour cet appareil.
              </td>
            </tr>
          )}
          {lignes.map((rapport) => (
            <tr key={rapport.id} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-3 whitespace-nowrap">{rapport.technicienNom}</td>
              <td className="px-4 py-3 whitespace-nowrap">
